  ...
  return (
    <Accordion className='reviewOutput'>
      <AccordionSummary
        ...
        id="panel1a-header">
        <Typography className='outputReviewTitle'>{props.books.bookTitle}</Typography>
        <Rating className='outputReviewRating' name="read-only" value={props.books.starRating} sx={{ ml: 5 }} readOnly />
        <Button className='deleteReview' variant="outlined" size="small" sx={{ ml: 120 }} startIcon={<DeleteIcon />} onClick={() => props.deleteReview(props.books.id)}>
          Delete
        </Button>
      </AccordionSummary>
      <AccordionDetails>
