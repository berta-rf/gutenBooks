...
function Reviewpage() {
  ...
  const addReview = (bookTitle, bookDescription, starRating) =>{
    ...
    let books = {id:id, bookTitle:bookTitle,bookDescription:bookDescription, starRating:starRating}
  ...
  return (
    ...
    {review.map((books)=> {
      return(
        ...
        <ReviewItem books={books} deleteReview={deleteReview} key={books.id}/>
      )
    })}
    </div>
